...
<body>
    ...
    $rand = rand(0, 100);
    if (isset($_POST['sua_don_dangky'])) {
        $MaTK = $_POST['ten_chu_nha'];
        $HinhTT = $_POST['hinh_tt_cu'];
        $goi_id = $_POST['goi'];
        ...
        if ($HinhTT_tmp == "") {
            ...
            if($HinhTT !=''){
                $up = mysqli_query($conn, "UPDATE tb_dondk SET MaTK =$MaTK,NgayDK = '$current_date', NgayHetHan = '$ngay_hethan', MaGoi = $goi_id, TrangThai = 1");
                if($up){
                    $_SESSION['msg_succ'] = 'Sửa đơn đăng ký thành công';
                        echo "<script>window.location.href='../view/admin/?page=don_dangky'</script>";
                } else {
                    $_SESSION['msg_err'] = 'Sửa đơn đăng ký thành không công';
                    echo "<script>window.location.href='../view/admin/?page=sua_don_dangky'</script>";
                }
            }
        } else {
            $new_hinh_tt = $rand . $HinhTT_name;
            if($HinhTT !=''){
                unlink( $uploaddir.$HinhTT);
            }
            if (file_exists($uploaddir . $new_hinh_tt)) {
                ...
                if ($move_up == true) {
                    $sql = "UPDATE tb_dondk SET MaTK =$MaTK,HinhTT = '$new_hinh_tt', NgayHetHan = '$ngay_hethan', MaGoi = $goi_id";
                    ...
                    if ($query) {
                        ...
                        $_SESSION['msg_err'] = 'Sửa đơn đăng ký không thành công';
                        echo "<script>window.location.href='../view/admin/?page=them_don_dangky'</script>";
                    }
                }
            }
